...
def _default_metadata(returns_interval: Timedelta) -> Dict[str, Dict[Timedelta, List[pl.Expr]]]:
...
class MetadataEngine(ByDateDataset):
    ...
    last_event_time_expr: pl.Expr = field(default_factory=lambda: pl.col('last_event_time'))

    # Additional arguments for returns engine & query commands
    returns_engine_kwargs: Dict[str, Any] = field(default_factory=dict)
    returns_query_kwargs: Dict[str, Any] = field(default_factory=lambda: {'filter_by_query_dates': True})
    ...
    metadata_exprs: Dict[str, Dict[Timedelta, List[pl.Expr]]] = field(default_factory=lambda: _default_metadata(Timedelta(minutes=10)))

    # Polars selector for metadata which should be quantile-expanded
    quantile_expand_exprs: pl.Expr = field(default_factory=lambda: pl.col('^daily_vol.*$', '^liquidity.*$'))
    ...
    def __post_init__(self):
        ...
        self.returns_engine = ReturnsEngine(self.backend_db, **self.returns_engine_kwargs)
        ...
        backend_universe_df = self.backend_dataset.cast_symbol_col_to_enum(self.backend_dataset.universe())
        self.returns_grid = backend_universe_df.with_columns(
    ...
    def _compute_partitions(self, dates=List[Date], verbose_debug: bool = None) -> pl.LazyFrame:
        """
        ...
        printv_lazy(lambda: f'Computing metadata for date range: {start_date} to {end_date}', verbose_debug)
        printv_lazy(lambda: f'  max_returns_lookback: {self.max_returns_lookback}, max_metadata_lookback: {self.max_metadata_lookback}', verbose_debug)
        ...
        printv_lazy(lambda: f'Step 1a - returns_query schema: {returns_query.collect_schema()}', verbose_debug)
        ...
        index_with_returns = self.returns_engine.query(
            ...
            append_lag=True, **self.returns_query_kwargs
        ).sort('symbol', 'returns_grid_time')
        printv_lazy(lambda: f'Step 1b - index_with_returns schema: {index_with_returns.collect_schema()}', verbose_debug)
        ...
        returns_metadata = pl.concat([
            ...
            grid_time = pl.col('returns_grid_time').dt.truncate(self.grid_interval) + self.grid_interval
        ...
        printv_lazy(lambda: f'Step 2 - returns_metadata schema: {returns_metadata.collect_schema()}', verbose_debug)
        ...
        printv_lazy(lambda: f'Step 3a - inrange_db schema: {inrange_db.collect_schema()}', verbose_debug)

        rolling_metadata = pl.concat([
            inrange_db.select('symbol', 'date', self.last_event_time_expr)
        ] + [
            inrange_db.rolling(
                self.last_event_time_expr, period=interval, closed='left', group_by='symbol'
            ).agg(cols).sort('symbol', self.last_event_time_expr).drop('symbol', self.last_event_time_expr)
            for interval, cols in self.metadata_exprs['by_symbol_index'].items()
        ], how='horizontal').with_columns(
            # Add grid_interval to make grid_time point-in-time as well
            grid_time = self.last_event_time_expr.dt.truncate(self.grid_interval) + self.grid_interval
        ).filter(
            # We can filter early here since here's a direct join to the final result
            pl.col('grid_time').is_between(start_date, end_date, closed='left')
        ).group_by('symbol', 'grid_time').agg(pl.all().last()).sort('symbol', 'grid_time')
        printv_lazy(lambda: f'Step 3b - rolling_metadata schema: {rolling_metadata.collect_schema()}', verbose_debug)
        ...
        final_metadata = rolling_metadata.join(
            ...
            (
                self.quantile_expand_exprs.rank('average') / self.quantile_expand_exprs.count()
            ).name.suffix('_q').over('grid_time')
        ).sort('symbol', self.last_event_time_expr).rename({'grid_time': 'time'})
        printv_lazy(lambda: f'Step 4 - final_metadata schema: {final_metadata.collect_schema()}', verbose_debug)
