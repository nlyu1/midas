...
class ReturnsEngine:
    def __init__(self, 
            db: pl.LazyFrame, 
            backend_fair_expr: pl.Expr = pl.col('vwap_price'), 
            backend_time_expr: pl.Expr = pl.col('last_event_time'), 
    ):
    ...
    def query(self,
        ...
        """
        Query for computing a single return specification.

        This is a convenience wrapper around query_batch that maintains the original API.
        ...
        Appended columns:
            - `max_tick_to_query_lag`: maximum lag from tick to specified query time, over start and end
            - `return`: computed return value
        """

        # Call query_batch with a single mark spec using temporary name '0'
        result = self.query_batch(
            query_lf,
            mark_exprs={'0': (start_time_expr, mark_duration)},
            tick_lag_tolerance=tick_lag_tolerance,
            append_query_tick_times=append_query_tick_times,
            append_lag=append_lag,
            filter_by_query_dates=filter_by_query_dates,
            append_start_end_fairs=append_start_end_fairs,
            verbose_debug=verbose_debug
        )

        # Strip '_0' suffix from all appended columns
        # Columns like 'return_0' -> 'return', 'max_tick_to_query_lag_0' -> 'max_tick_to_query_lag'
        return result.rename({
            col: col[:-2]  # Remove last 2 characters '_0'
            for col in result.collect_schema().names() if col.endswith('_0')
        })
    ...
    def query_batch(self,
        ...
        wide_append_cols = append_cols.group_by('row_id').agg(*[
            pl.all().exclude('row_id', 'return_col_name').name.suffix(
                f'_{return_col}'
            ).filter(pl.col('return_col_name') == return_col).first()
            for return_col in mark_exprs.keys()
        ])
